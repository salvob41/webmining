...
#    - This is ok for small dataset sizes. For the full 10,000 songs, it
#      takes 20 minutes!
...
# Create a dictionary of the songs, mapping the article identifier (e.g.,
...
for i in range(0, numDocs):
    ...
    words = [word for text in s for word in nltk.word_tokenize(text)]
...
for i in range(0, numDocs):
    ...
    for j in range(i + 1, numDocs):
        ...
        estJSim[getTriangleIndex(i, j)] = float(count / numHashes)
...
for i in range(0, numDocs):
    if docNames[i] not in distances:
        distances[docNames[i]] = dict()
    i_results = 0
    for j in range(i + 1, numDocs):
        # We have a triangular matrix.. to have them both way
        if docNames[j] not in distances:
            distances[docNames[j]] = dict()
        # Retrieve the estimated similarity value for this pair.
        estJ = estJSim[getTriangleIndex(i, j)]
        distances[docNames[i]].update({docNames[j]:estJ})
        distances[docNames[j]].update({docNames[i]:estJ})

        # If the similarity is above the threshold...
        if estJ > threshold and i_results<max_results:
            ...
            if plagiaries[docNames[i]] == docNames[j]:
                ...
                fp = fp + 1
            '''
    connected_songs = distances[docNames[i]]
    output = [(k, connected_songs[k]) for k in sorted(connected_songs, key=connected_songs.get, reverse=True)][:20]
    distances[docNames[i]] = dict(output)

# Display true positive and false positive counts.
print()

with open("minhash_dist.json", "w") as f:
